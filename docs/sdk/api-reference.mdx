...
Methods for passwordless passkey authentication.

### `getWebAuthnRegistrationOptions(username)`

Get options for creating a new passkey credential.
...
**Parameters:**
- `username: string` - Username to associate with the passkey

**Returns:** `Promise<WebAuthnRegistrationOptions>` - Options for `navigator.credentials.create()`

**Example:**
```typescript
const options = await auth.getWebAuthnRegistrationOptions('john@example.com');
const credential = await navigator.credentials.create({ publicKey: options });
```

---

### `verifyWebAuthnRegistration(credential)`
...
**Parameters:**
- `credential: WebAuthnRegistrationCredential` - Credential from browser
...
Get options for authenticating with an existing passkey.

```typescript
async getWebAuthnAuthenticationOptions(): Promise<WebAuthnAuthenticationOptions>
```

**Returns:** `Promise<WebAuthnAuthenticationOptions>` - Options for `navigator.credentials.get()`
...
### `verifyWebAuthnAuthentication(credential)`

Verify passkey authentication and complete sign-in.
...
**Parameters:**
- `credential: WebAuthnAuthenticationCredential` - Assertion from browser
